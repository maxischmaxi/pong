...
A terminal UI pong tool with multi-host support, built with Rust and [Ratatui](https://ratatui.rs).
...
```bash
curl -fsSL https://raw.githubusercontent.com/maxischmaxi/pong/main/install.sh | sudo bash
```
...
```bash
git clone https://github.com/maxischmaxi/pong.git
cd pong
cargo build --release
